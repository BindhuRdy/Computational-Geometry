...
{	 
    ...
    //Triangulation ->DCEL creation->Hertel Mehlhorn Algorithm invocation for polygon decomposition
	public void Triangulate2() {
        System.out.println("Polygon decomposition starts");
		cVertex v0, v1, v2, v3, v4;   // five consecutive vertices 
		cDiagonal diag;
		int   n = listcopy.n;         //number of vertices; shrinks to 3
		boolean earfound = false;     //to prevent infinite loop on improper input
		...
		while ( n > 3 ) {
			...
			do {
				if (v2.ear) {
					...
					diag.PrintDiagonal(listcopy.n - n);
					diaglist.InsertBeforeHead( diag );
                    //add the diagonal obtained to dcel.
					dcel.addDiagonal(v1, v3);
			...
			if (!earfound) {
				System.out.println("Polygon is nonsimple: cannot triangulate and perform hertel mehlhorn");
				break;
			}
			else
				earfound = false;
        ...
        //apply hertel mehlhorn algorithm to the ear diagonals stored in dcel
		dcel.hertelMehlhorn();	    	  
	}
